...
v1.12。仕様書のPhase 1〜3を実装済み。Phase 4はDaily Challengeのみ着手。

### v1.12で変えたこと（確認手段）
- `buildConfig = true` を有効にし、ホーム画面とPAUSEダイアログに `BuildConfig.VERSION_NAME` を表示。
  端末で動いているビルドが一目で分かる。ホームには現行ルール（キング開放 最大4 / 配札はいつでも可）も併記。
- ルール自体の実装は v1.11 から変更なし。v1.11 の内容が反映されていない場合は、
  端末のAPKが更新されていない可能性を先に疑う。
